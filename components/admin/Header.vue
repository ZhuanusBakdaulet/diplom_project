<template>
  <div class="d-flex align-center justify-space-between header">
    <nuxt-link to="/admin" class="h1 gilroy text-uppercase blue-color">
      Admin
    </nuxt-link>
    <nuxt-link to="/Profile" class="h3 d-flex align-center">
      <svg
        class="mr-2"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M11.9849 15.3462C8.11731 15.3462 4.81445 15.931 4.81445 18.2729C4.81445 20.6148 8.09636 21.2205 11.9849 21.2205C15.8525 21.2205 19.1545 20.6348 19.1545 18.2938C19.1545 15.9529 15.8735 15.3462 11.9849 15.3462Z"
          stroke="#6BB4FF"
          stroke-width="1.5"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M11.9849 12.0059C14.523 12.0059 16.5801 9.94779 16.5801 7.40969C16.5801 4.8716 14.523 2.81445 11.9849 2.81445C9.44679 2.81445 7.3887 4.8716 7.3887 7.40969C7.38013 9.93922 9.42394 11.9973 11.9525 12.0059H11.9849Z"
          stroke="#6BB4FF"
          stroke-width="1.42857"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
      </svg>

      Личный кабинет
    </nuxt-link>
  </div>
</template>

<script>
export default {};
</script>

<style lang="scss" scoped>
@import "@/assets/css/admin.scss";
.header {
  width: 100%;
  height: 60px;
  padding-left: 80px;
  padding-right: 98px;
  background: #ffffff;
  box-shadow: 0px 4px 10px rgba(119, 119, 119, 0.1);
  z-index: 10;
}
a {
  text-decoration: none;
  color: $black-color;
}
</style>
